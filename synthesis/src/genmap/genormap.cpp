...
{
    struct Or2Predicate : public InstancePredicate
    {
        virtual bool operator()(const SyModInst *inst) const {
            return inst->getMasterName() == string("OR_2_1");
        }    
    };
    GenericOrMapper::GenericOrMapper() {
    }

    GenericOrMapper::GenericOrMapper(SBlock *block) :
    ...
    GenericOrMapper*
    GenericOrMapper::create(SBlock *block) const{
        return new GenericOrMapper(block);
    }    

    const SCell*
    GenericOrMapper::getCell() const
    {
        const SCell *cell = SCellMgr::instance()->getCell(SCell::OR, 2);
        assert(cell);
        return cell;
    }

    void
    GenericOrMapper::setPinMapList()
    {
    ...
    void
    GenericOrMapper::doMapping() {
        this->createCellInstances(Or2Predicate());
    }    
