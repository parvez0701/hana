...
namespace Synth
{
    ...
    {
        public:
            typedef std::map<std::string, int, cmp_str> PrefixCountMap;
            typedef std::map<std::string, PrefixCountMap, cmp_str> ModuleMap;

            static NameMgr *instance();


            std::string createName(const T*, const std::string&);
        private:
            NameMgr() {
            }
            static NameMgr *nameMgr;
            DISABLE_COPYCONST_ASSGNOP(NameMgr);
            ModuleMap modMap;
    };
    ...
    typedef NameMgr<SyModule> ModuleNameManager;
    typedef NameMgr<stview::SBlock> BlockNameManager;
